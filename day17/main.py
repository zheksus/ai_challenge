...
class MemoryManager:
    ...
    def get_full_context(self, include_short_term: bool = True,
                         ...
                         profile_prompt: str = "",
                         task_context: str = "",
                         mcp_prompt: str = "") -> List[Dict[str, str]]:
        """
...
class MemoryAwareAgent:
    ...
    def __init__(self, auth_key: str, model: str = "GigaChat", session_id: str = "default"):
        ...
        # Пытаемся подключиться к MCP (не критично, если не работает)
        try:
            self.mcp_connected = self.mcp.connect()
            if self.mcp_connected:
                self.mcp_tools = self.mcp.get_tools()
                print(f"📦 MCP инструменты: {[t['name'] for t in self.mcp_tools]}")
        except Exception as e:
            print(f"⚠️ MCP не подключён: {e}")
    ...
    def call_mcp_tool(self, tool_name: str, arguments: dict) -> dict:
        ...
        if not self.mcp_connected:
            return {"error": "MCP не подключён. Запустите weather_mcp_server.py"}
    ...
    def _format_weather_result(self, result: dict) -> str:
        """
        Форматирование результата погоды для пользователя.
        """
        if "error" in result:
            return f"❌ {result['error']}"

        city = result.get("city", "Неизвестный город")
        temp = result.get("temperature", {}).get("value", "N/A")
        units = result.get("temperature", {}).get("units", "°C")
        condition = result.get("condition", "N/A")
        humidity = result.get("humidity", "N/A")
        wind = result.get("wind", {}).get("speed", "N/A")
        wind_units = result.get("wind", {}).get("units", "км/ч")
        source = result.get("source", "")

        lines = [
            f"🌤️ **Погода в {city}**",
            "",
            f"🌡️ Температура: {temp}{units}",
            f"🌥️ Условия: {condition}",
            f"💧 Влажность: {humidity}",
            f"💨 Ветер: {wind} {wind_units}"
        ]
    ...
    def ask(self, question: str, temperature: float = 0.7) -> tuple:
        ...
        # ===== ОБЫЧНОЕ СООБЩЕНИЕ =====
            ...
            # Проверяем, не нарушает ли запрос инварианты
    ...
    Пожалуйста, измените запрос с учётом инвариантов."""
        ...
        mcp_prompt = self.mcp.get_tools_prompt() if self.mcp_connected else ""
        ...
        if "choices" in response and len(response["choices"]) > 0:
            ...
            if "погод" in answer.lower() or "weather" in answer.lower():
                # Проверяем, есть ли вызов инструмента
                import re
                tool_pattern = r'(get_current_weather|get_weather_forecast)\s*\(\s*([^)]+)\s*\)'
                matches = re.findall(tool_pattern, answer)

                if matches and self.mcp_connected:
                    for tool_name, args_str in matches:
                        ...
                        for pair in args_str.split(','):
                            if '=' in pair:
                                ...
                                # Если просто город без ключа
                                args['city'] = pair.strip().strip('"\'')

                        # Вызываем MCP инструмент
                        result = self.call_mcp_tool(tool_name, args)
                        if "error" not in result:
                            formatted = self._format_weather_result(result)
                            return formatted, {"mcp_called": True, "tool": tool_name}
