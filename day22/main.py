...
Отвечай по делу, без лишних пояснений."""
    ...
    def ask_with_rag(self, question: str, strategy: str = "structure",
                     top_k: int = 5) -> str:
        ...
        context = self.rag_indexer.query_to_context(question, strategy=strategy, top_k=top_k)
        if not context:
    ...
    def run_rag_comparison(self, questions: List[Dict] = None) -> str:
        """Запуск сравнения с RAG и без RAG по списку вопросов."""
        ...
        lines = []
        lines.append("=" * 100)
        lines.append("📊 СРАВНЕНИЕ: БЕЗ RAG vs С RAG")
        lines.append("=" * 100)
        ...
        lines.append(f"\n{'=' * 100}")
        lines.append("✅ Сравнение завершено. Оцените качество ответов визуально.")
        lines.append(f"{'=' * 100}")
...
class MemoryTestCLI:
    ...
    def _print_help(self):
        ...
        print("  /ask <вопрос>      - ответ С RAG (поиск по индексу + LLM)")
        print("  /ask0 <вопрос>     - ответ БЕЗ RAG (только LLM)")
        print("  /compare           - сравнить RAG vs без RAG на 10 вопросах")
            ...
            try:
                ...
                if user_input.startswith("/search "):
                    ...
                    if not self.agent.rag_indexer.indexes:
                        ...
                        continue
                    strategy = "structure" if "structure" in self.agent.rag_indexer.indexes else "fixed_size"
                ...
                if user_input == "/rag":
                    ...
                    print(f"    /ask <вопрос>    — RAG-запрос к LLM")
                    print(f"    /ask0 <вопрос>   — запрос без RAG")
                    print(f"    /compare         — сравнить RAG vs без RAG (10 вопросов)")
                    continue
                ...
                if user_input.startswith("/ask "):
                    ...
                    print(f"\n🔍 RAG-ЗАПРОС: {question}")
                    print("=" * 70)
                    answer = self.agent.ask_with_rag(question)
                    print(f"\n🤖 Ответ (с RAG):\n{answer}")
                    continue

                if user_input == "/ask0 " or user_input.startswith("/ask0 "):
                    question = user_input[6:].strip() if user_input.startswith("/ask0 ") else ""
                    if not question:
