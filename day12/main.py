...
from day12.profile import ProfileManager
...
class MemoryManager:
    ...
    def get_full_context(self, include_short_term: bool = True,
                         ...
                         profile_prompt: str = "") -> List[Dict[str, str]]:
        """
...
class MemoryAwareAgent:
    ...
    def ask(self, question: str, temperature: float = 0.7) -> Tuple[str, Dict]:
        ...
        # 👇 ПОЛУЧАЕМ ПРОФИЛЬ И ФОРМИРУЕМ КОНТЕКСТ
        ...
        context = self.memory.get_full_context(
            ...
            last_n_messages=10,
            profile_prompt=profile_prompt  # 👈 ПЕРЕДАЁМ ПРОФИЛЬ
...
class MemoryTestCLI:
    ...
    def _print_help(self):
        ...
        print("  /profile reset - сбросить профиль (запустить опросник заново)")
        ...
        while True:
            try:
                ...
                if user_input == "/exit":
                    ...
                    print("\n" + self.agent.profile_manager.show_profile())
                elif user_input == "/profile edit":
                    print("\n📝 Редактирование профиля (введите новые значения или Enter для пропуска):")
