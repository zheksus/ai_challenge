...
Отвечай по делу, без лишних пояснений."""
    ...
    def ask_with_rag(self, question: str, strategy: str = "structure",
                     ...
                     use_rewrite: bool = False, min_score: float = None) -> str:
    ...
    def run_rag_comparison(self, questions: List[Dict] = None) -> str:
        ...
        lines = []
        lines.append("=" * 120)
        lines.append("📊 СРАВНЕНИЕ РЕЖИМОВ RAG: без RAG | без фильтра | с фильтром | с rewrite")
        lines.append("=" * 120)
        ...
        for i, item in enumerate(questions, 1):
            ...
            no_filter = self.ask_with_rag(q, use_filter=False)
            with_filter = self.ask_with_rag(q, use_filter=True, min_score=0.25)
            with_rw = self.ask_with_rag(q, use_filter=True, use_rewrite=True, min_score=0.25)

            lines.append(f"\n  🚫 Без RAG:      {without[:150]}")
            lines.append(f"\n  📖 Без фильтра:  {no_filter[:150]}")
            lines.append(f"\n  ✅ С фильтром:   {with_filter[:150]}")
            lines.append(f"\n  🔄 С rewrite:    {with_rw[:150]}")
        ...
        while True:
            try:
                ...
                if user_input == "/rag":
                    ...
                    print(f"    /ask <вопрос>    — RAG + фильтр")
                    print(f"    /ask_nf <вопрос> — RAG без фильтра")
                    print(f"    /ask_rw <вопрос> — RAG + rewrite + фильтр")
                    print(f"    /ask0 <вопрос>   — без RAG")
                    print(f"    /compare         — сравнить 4 режима")
