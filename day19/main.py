...
class MemoryAwareAgent:
    ...
    def __init__(self, auth_key: str, model: str = "GigaChat", session_id: str = "default",
        ...
        # 👇 MCP-клиент для вызова инструментов погоды
    ...
    MCP_CALL_RE = re.compile(r'\[MCP:(\w+)\s*(.*?)\]')

    def _try_connect_mcp(self):
        try:
            if self.mcp.connect():
                self.mcp_connected = True
                self.mcp_tools = self.mcp.fetch_tools()
                print(f"🔌 [MCP] Connected. Tools: {[t['name'] for t in self.mcp_tools]}")
            else:
                print("⚠️ [MCP] Не удалось подключиться к MCP-серверу")
        except Exception as e:
            print(f"⚠️ [MCP] Ошибка подключения: {e}")

    def _get_mcp_tools_prompt(self) -> str:
        if not self.mcp_tools:
            return ""
        lines = [
            "### MCP-ИНСТРУМЕНТЫ (доступны для вызова):",
            "Для получения данных о погоде используй инструменты ниже."
        ]
...
class MemoryTestCLI:
    ...
    def _print_help(self):
        ...
        print("\n📌 MCP-ИНТЕГРАЦИЯ (погода):")
        print("  /mcp               - показать статус MCP и доступные инструменты погоды")
        print("  💡 Просто спросите 'какая погода в Москве' — агент сам вызовет MCP")
    ...
    def _print_mcp(self):
        ...
        if not self.agent.mcp_connected:
            print("❌ MCP-сервер не подключён")
            print(f"   URL: {self.agent.mcp.mcp_url}")
            print("   Запустите weather_mcp_server.py и перезапустите агента")
        else:
            print(f"✅ Подключён к {self.agent.mcp.mcp_url}")
            print(f"📦 Доступно инструментов: {len(self.agent.mcp_tools)}")
            print()
            for t in self.agent.mcp_tools:
                name = t["name"]
                desc = t["description"]
