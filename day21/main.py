    ...
    def run(self):
        ...
        while True:
            try:
                ...
                # ===== RAG (ИНДЕКСАЦИЯ) =====
                if user_input == "/index":
                    print("\n📚 Запуск полной индексации (обе стратегии)...")
                    result = self.agent.rag_indexer.build_index(strategy="both")
                    if "fixed_size" in result and "structure" in result:
                        print(compare_strategies(result["fixed_size"], result["structure"]))
                    continue
